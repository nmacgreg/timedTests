...
my $maxQuotient = sqrt $maxProduct;  # figure out which times-table we're focusing on, in this test
...
for ($maxQuestions = 1; $maxQuestions<31; $maxQuestions++) {
	...
	if ((defined $maxDiv) and (($randA * $randB) <= $maxDiv)) { 
		$randC=int(rand(2)); # select addition or multiplication
		if ( $randC == 1  ) {
			print "$maxQuestions. " . $randA * $randB .  " ÷ 9 =\n";
		}else {
